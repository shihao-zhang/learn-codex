...
本章回答“能力如何进入 agent 平台”的设计问题，但当前只确认路径存在已核实，能力语义待核实。

已登记的公开源码路径包括 MCP handler、extension tools handler、`skills` crate 目录。它们说明这些代码入口存在；它们尚不足以证明 MCP、extensions、skills 在 Codex CLI 中分别是什么用户可见能力、生命周期如何、是否属于同一扩展体系。因此本章不会声称“skills 是 CLI 一等能力”，也不会把 Claude Code 或其他 agent 产品的 skills 概念套到 Codex 上。
...
见 [diagram.mmd](diagram.mmd)。图把 MCP、extension tools、skills 先画成“待核实的能力来源线索”，再进入发现、工具规范、治理和运行时；虚线表示语义待核实。
...
- `MCP`：待核实路径指向 MCP handler。产品设计上应重点核实工具命名、schema 转换、调用结果、审批、并行安全和错误回传。
- `extension tools`：待核实路径指向 extension tools handler。设计上要关注它是如何把外部 executor 适配进统一工具表面，以及是否有独立生命周期。
- `dynamic tools`：概念上指“运行时才发现或暴露的工具”。需要核实其来源、加载时机、可见范围和是否受同一权限框架约束。
- `skills directory`：路径存在已核实，能力语义待核实。此处只能把它当作源码线索，不能写成 Codex CLI 已确认的用户级 skills 功能。
...
- 教学 mock 会把所有扩展统一成同一种 `ToolSpec`，真实实现可能有不同 adapter、事件、权限和生命周期。
...
- [x] 当前只说 MCP、extension tools、skills 相关路径存在已核实。
