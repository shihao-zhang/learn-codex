...
- 可见图中包含图例或边界说明。
- 提交前必须渲染为图片或浏览器预览，确认主路径、侧向路径、文字、chip、箭头和图例可读。
...
- 主阅读路径必须一眼可见。包含 `FACT` 主干、教学 failure path、恢复路径和图例时，优先把它们拆成“主路径、侧向解释、图例”三层，而不是让所有节点同级铺开。
- `FACT` 主干通常应占据最稳定的横向或纵向阅读轴；教学示例、mock trace、恢复文案和图例应降为次级区域。
- 失败、拒绝和恢复路径只在解释阻塞原因时强调，不应让颜色或箭头强度压过主机制路径。
- 模块、文字、chip、箭头和图例之间必须留出可见空隙；连线不得穿过读者需要阅读的文字。
- 如果渲染后出现模块重叠、文字遮挡、箭头压字、图例过重或第一阅读路径不清，应先减节点、减 chip 或重排层级，再继续补充内容。
...
- [ ] 是否已渲染预览，并确认没有模块重叠、文字遮挡、箭头压字或图例抢占主路径。
- [ ] 主路径是否一眼可见；教学示例、failure path、恢复路径和图例是否处在次级区域。
